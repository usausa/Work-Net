namespace WorkWatTest;

using System.Diagnostics;
using System.Net.Sockets;

internal class Program
{
    public static void Main()
    {
        while (true)
        {
            using var client = new TcpClient();
            client.Connect("192.168.100.171", 60121);

            var cmd = MakeMeasureValueCommand(0);
            //Debug.WriteLine(Convert.ToHexString(cmd));
            client.Client.Send(cmd);

            var array = new byte[256];
            var readed = 0;
            while (true)
            {
                var read = client.Client.Receive(array);
                if (read < 0)
                {
                    break;
                }

                if (read > 0)
                {
                    readed += read;
                    //Debug.WriteLine($"{readed} {Convert.ToHexString(array, 0, readed)}");
                    var num47 = array[1] & 0xFF;
                    if (num47 == 0)
                    {
                        var voltage = (double)(((long)array[10] << 40) + ((long)array[9] << 32) + ((long)array[8] << 24) + ((long)array[7] << 16) + ((long)array[6] << 8) + array[5]) / (1L << 24);
                        var current = (double)(((long)array[16] << 40) + ((long)array[15] << 32) + ((long)array[14] << 24) + ((long)array[13] << 16) + ((long)array[12] << 8) + array[11]) / (1L << 30);
                        var power = (double)(((long)array[22] << 40) + ((long)array[21] << 32) + ((long)array[20] << 24) + ((long)array[19] << 16) + ((long)array[18] << 8) + array[17]) / (1L << 24);
                        var dateTime= new DateTime(array[28] + 2000, array[27], array[26], array[25], array[24], array[23]);

                        Console.WriteLine($"{dateTime:yyyy/MM/dd HH:mm:ss}: 電力={power:F3}W, 電圧={voltage:F3}V, 電流={current * 1000.0:F3}A");
                    }

                    break;
                }

                Thread.Sleep(100);
            }

            Thread.Sleep(1000);
        }
    }

    public static byte[] MakeMeasureValueCommand(byte flg)
    {
        return MakeCommand([24, flg]);
    }

    public static byte[] MakeCommand(byte[] payload)
    {
        var array = new byte[payload.Length + 4];
        array[0] = 0xAA;
        array[1] = (byte)((uint)(payload.Length >> 8) & 0xFF);
        array[2] = (byte)((uint)payload.Length & 0xFFu);
        payload.CopyTo(array, 3);
        array[^1] = CalcCrc8(payload);
        return array;
    }

    public static byte CalcCrc8(byte[] payload)
    {
        var array = new[]
        {
            
        }; var num = 0;
        foreach (var b in payload)
        {
            num = array[(num ^ b) & 0xFF];
        }
        return (byte)num;
    }
}
