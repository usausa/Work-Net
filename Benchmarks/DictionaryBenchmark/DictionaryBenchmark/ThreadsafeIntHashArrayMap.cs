namespace Smart.Collections.Concurrent
{
    ...
    public sealed class ThreadsafeIntHashArrayMap<TValue> : IEnumerable<KeyValuePair<int, TValue>>
    {
        private static readonly Node EmptyNode = new Node(-1, default);
        ...
        public ThreadsafeIntHashArrayMap(int initialSize = 64, double factor = 3.0)
            : this(new GrowthHashArrayMapStrategy(initialSize, factor))
        {
        }

        public ThreadsafeIntHashArrayMap(IHashArrayMapStrategy strategy)
        {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1062:ValidateArgumentsOfPublicMethods", Justification = "Performance")]
        public TValue AddIfNotExist(int key, TValue value)
        {
            lock (sync)
            {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1062:ValidateArgumentsOfPublicMethods", Justification = "Performance")]
        public TValue AddIfNotExist(int key, Func<int, TValue> valueFactory)
        {
            lock (sync)
            {
                // Double checked locking
        ...
        public IEnumerator<KeyValuePair<int, TValue>> GetEnumerator()
        {
            ...
            for (var i = 0; i < copy.Length; i++)
            {
                ...
                if (node != EmptyNode)
                {
                    do
                    {
                        yield return new KeyValuePair<int, TValue>(node.Key, node.Value);
                        node = node.Next;
                    } while (node != null);
                }
            }
