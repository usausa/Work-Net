...
internal class Program
{
    ...
    {
        await using var client = new PostgresClient("192.168.100.73", 5432, "test", "test", "test");
        ...
        try
        {
            ...
            var newUser = new User(1001, "Test User", "test@example.com", DateTime.Now);
            var insertCount = await client.InsertUserAsync(newUser);
            Console.WriteLine($"挿入: {insertCount} 行\n");

            // 2. Read - ユーザー取得
            Console.WriteLine("=== READ (単一) ===");
            var user = await client.GetUserByIdAsync(1001);
            if (user != null)
                Console.WriteLine($"ID: {user.Value.Id}, Name: {user.Value.Name}, Email: {user.Value.Email}, Created: {user.Value.CreatedAt}\n");

            // 3. Read - 全ユーザー取得
            Console.WriteLine("=== READ (全件) ===");
            var users = await client.GetAllUsersAsync();
            Console.WriteLine($"ユーザー数: {users.Count}\n");

            // 4. Update - ユーザー更新
            Console.WriteLine("=== UPDATE ===");
            var updateCount = await client.UpdateUserAsync(1001, "Updated User", "updated@example.com");
            Console.WriteLine($"更新: {updateCount} 行\n");

            // 確認
            Console.WriteLine("=== READ (更新確認) ===");
            user = await client.GetUserByIdAsync(1001);
            if (user != null)
                Console.WriteLine($"ID: {user.Value.Id}, Name: {user.Value.Name}, Email: {user.Value.Email}\n");

            // 5. Delete - ユーザー削除
            Console.WriteLine("=== DELETE ===");
            var deleteCount = await client.DeleteUserAsync(1001);
            Console.WriteLine($"削除: {deleteCount} 行\n");

            // 確認
            Console.WriteLine("=== READ (削除確認) ===");
            user = await client.GetUserByIdAsync(1001);
            Console.WriteLine(user == null ? "ユーザーが見つかりません\n" : $"ユーザー: {user.Value.Name}\n");
